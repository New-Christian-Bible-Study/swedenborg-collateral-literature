...
AsciiDoc documentation begins here:
...
should be highlighted, only in the source,
perhaps by colored squiggly underlining

....
////
comment block bounded by 4 slashes
ok
////
....
...
==== BLOCKS: delimited, can be nested, not overlapped.
Go ahead and play with these.

====
4+ equal signs are bounded .... #_are **stylized**_#
====

----
4+ dashes or dots are bounded .... #_are **verbatim**_# (RAW)
----

--
2 dashes are UNbounded .... #_are **stylized**_#
--

++++
4+ plus signs are UNbounded .... #_are **verbatim**_# (raw)
++++

//.... _Dots initialize a *List*_, which can be a pain if you don't want a list.

---

....
 _Italicize with underbars_ ---
 *Bold with asterisks* ---
 `Monospace with back-ticks` ---
 #Highlight with a pair of number/pound signs# ---
 ~Subscript~ with tilde, and ^Superscript^ with carats ---
*_Roles_*, such as [.underline]#underline#, are delimited by the number (#) sign, and declared by naming the role, preceded by a dot, within square brackets, preceeding the initial delimiter.
....

[.text-center]
Note the block delimiters used above in the source for below
...
#Highlight with a pair of number/pound signs# ---
~Subscript~ with tilde, and ^Superscript^ with carats ---
*_Roles_*, such as [.underline]#underline#, are delimited by the number (#) sign, and declared by naming the role, preceded by dot, within square brackets, preceeding the initial delimiter.

ref. https://docs.asciidoctor.org/asciidoc/latest/text/text-span-built-in-roles/

NOTE: "the style/theming system of the converter can allow you to define styles for a custom role." 
...
In some cases(*1), there is a need for multiple footnotes(*2) in the same line.
 ...
 Even with single notes, it can be a good look to indent these
...
==== ##[.underline]#a horizontal line separator,### generated with three dashes following a blank line, as below:
...
NOTE:
Editors with access to authoritative sources are encouraged to seek and resolve these mysteries within the entire collection. (by searching for literal unquoted character pair '[?')
